...
public class Purchase extends JFrame {
	...
	public Purchase() {
		...
		JLabel lblHeading = new JLabel("PURCHASE");
		...
		try {
			...
			pstate=dbcon.prepareStatement("select * from purchase");
			ResultSet rs=pstate.executeQuery();
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(cat<=0) {
					...
					try {
						...
						dbcon=mcon.getConnection();
						pstate=dbcon.prepareStatement("insert into purchase values(?,?,?,?,?,?,?,?)");
						...
						if(i>0) {
							JOptionPane.showMessageDialog(null,"Product purchased successfully.","Success",JOptionPane.INFORMATION_MESSAGE);
